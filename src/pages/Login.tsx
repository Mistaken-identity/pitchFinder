    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="max-w-6xl w-full flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-24 relative z-10 transition-all duration-700">
        ...
        <motion.div 
          ...
          className={`flex-1 flex flex-col items-center justify-center text-center lg:text-left px-4 ${
        ...
        >
          <div className="relative w-full max-w-[280px] md:max-w-sm aspect-square flex items-center justify-center">
            <JugglingCharacter 
              avatarUrl={`https://api.dicebear.com/7.x/notionists/svg?seed=${role === 'player' ? 'Felix' : 'Owner'}&backgroundColor=0f172a`} 
          ...
          <div className="mt-6 lg:mt-12 max-w-sm mx-auto lg:mx-0">
            <div className="hidden sm:inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-emerald-500/5 border border-emerald-500/20 mb-4">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.25em] text-emerald-400">
                {role === 'player' ? 'Live Feedback Enabled' : 'Venue Analytics Active'}
              </span>
            </div>
            <h3 className="text-2xl md:text-5xl font-black italic tracking-tighter text-white mb-3 uppercase">
              {role === 'player' ? 'REJOIN THE' : 'ENTER YOUR'} <span className="neon-text">{role === 'player' ? 'SQUAD.' : 'HUB.'}</span>
            </h3>
            <p className="text-slate-400 text-sm md:text-base leading-relaxed font-medium">
              {role === 'player' 
                ? 'Experience the future of Kenyan semi-pro football. Your teammates are checking the clock.'
                : 'Manage your pitch schedule, track earnings, and respond to booking requests in real-time.'}
            </p>
          </div>
        </motion.div>
            ...
            <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 mb-8">
              <button
                ...
                className={`flex-1 py-3 px-4 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${
                  role === 'player' ? 'bg-emerald-500 text-slate-950 shadow-lg' : 'text-slate-400 hover:text-white'
                }`}
              >
                Player
              </button>
              <button
                ...
                className={`flex-1 py-3 px-4 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${
                  role === 'owner' ? 'bg-emerald-500 text-slate-950 shadow-lg' : 'text-slate-400 hover:text-white'
                }`}
              >
                Owner
              </button>
